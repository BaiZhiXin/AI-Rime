...
增加上下文感知功能，可在配置文件中开启，开启后会将之前输入的内容进行拆词并将结果作为上下文发送给AI，供其参考。在提示词设置合理情况下可对人称代词净准纠正。

由于该项目基本确定了使用临时文件的形式进行lua与服务程序间的通信，因此现在服务程序正式开源。服务程序以python为基础构建，需要安装python包：requests、pynput、jieba，分别用于发送AI请求、发送模拟按键及拆词。其中Linux操作系统往往不需要安装pynput，因为Linux下基于ydotool及xdotool实现模拟按键。

最后欢迎各位基于本项目进行二创出更好用的开源输入方案。
